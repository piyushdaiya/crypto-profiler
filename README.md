...
- curated dataset-mode scoring for Ethereum, Optimism, Polygon, Arbitrum, ERC-20, Solana, and Bitcoin cases
...
- Built to demonstrate multi-chain wallet intelligence without pretending Ethereum, Optimism, Polygon, Arbitrum, Solana, Bitcoin, and ERC-20 all share the same data model.
...
- curated dataset-mode scoring for Optimism, Polygon, Arbitrum, ERC-20, Solana, and Bitcoin
...
- Optimism, Polygon, Arbitrum, Solana, Bitcoin, and ERC-20 are real scoped expansions, but currently delivered through dataset mode
- Optimism, Polygon, and Arbitrum Phase 1 are intentionally transactions-only because decoded-events were evaluated and deferred due scan-cost / ROI
...
### Arbitrum Layer 2 (Phase 1)

Arbitrum Phase 1 is currently implemented in dataset mode using a transactions-only workflow.
...
- shortlist export to Parquet
- homelab-first local summarization
- curated Arbitrum cases under `data/cases/curated-arbitrum/`
- validator dataset-mode scoring for:
  - repeated-contract, service-like behavior
  - broad operational hub behavior
- analyst-facing report output for Arbitrum cases

Important implementation note:

- Phase 1 is intentionally transactions-only
- decoded-events were evaluated but deferred for now because scan-cost / ROI was weak relative to the value needed for the first pass
- this keeps the Arbitrum path practical within a homelab-first, credits-conscious workflow

Current curated Arbitrum cases:
...
- Arbitrum Phase 1 cases under `data/cases/curated-arbitrum/`
...
- Optimism, Polygon, and Arbitrum are currently dataset-mode only
...
- validator dataset-mode routing across Ethereum, Optimism, Polygon, Arbitrum, Solana, Bitcoin, and ERC-20 curated cases
- curated loader failure modes for malformed JSON and missing required fields
- chain-specific Solana, Bitcoin, ERC-20, Optimism, Polygon, and Arbitrum dataset scoring thresholds and differentiated reason generation
