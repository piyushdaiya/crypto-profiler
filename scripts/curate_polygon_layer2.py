...
#!/usr/bin/env python3
"""
curate_polygon_layer2.py

Transactions-only Phase 1 curator for Polygon.
...
HUB_ADDRESS = "0x2b92478666516ef5d997490981aa3e00e9c8da9a"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Curate Polygon Layer 2 Phase 1 cases")
...
def main() -> None:
    ...
    service_like_case = build_case(
        case_id="polygon-repeated-contract-service-like",
        title="Polygon Repeated Contract Service-Like Wallet",
        description="Polygon Layer 2 case showing extremely concentrated single-contract transaction behavior with constrained counterparty diversity.",
        ...
        selection_basis=(
            ...
            "diversity relative to total activity in a tx-only Phase 1 Polygon workflow."
        ),
    )

    hub_case = build_case(
        case_id="polygon-broad-operational-hub",
        title="Polygon Broad Operational Hub",
        description="Polygon Layer 2 case showing broad mixed-flow operational activity across a very large counterparty surface.",
        ...
        selection_basis=(
            "Selected because it combined extremely broad unique counterparties, balanced mixed flow direction, "
            "and very low dominant-counterparty concentration in a transactions-only Polygon Phase 1 workflow."
        ),
    )

    service_like_out = out_dir / "polygon-repeated-contract-service-like.json"
    hub_out = out_dir / "polygon-broad-operational-hub.json"
    ...
    manifest = {
        "script": "curate_polygon_layer2.py",
