...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Merge Arbitrum Phase 2 parquet summaries into extracted JSON artifacts"
    )
    ...
    parser.add_argument("--out-dir", required=True, help="Output directory for final Arbitrum Phase 2 extracts")
...
def load_payload_index(input_dir: Path) -> dict[str, dict[str, Any]]:
    ...
    for path in sorted(input_dir.glob("*.json")):
        ...
        if not address or chain != "ARBITRUM":
            continue
...
def main() -> None:
    ...
    manifest = {
        "script": "extract_arbitrum_phase2_semantics.py",
