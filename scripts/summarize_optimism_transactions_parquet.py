...
# scripts/summarize_optimism_transactions_parquet.py
#!/usr/bin/env python3
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Summarize exported Optimism transaction parquet files")
    parser.add_argument("--input-dir", required=True, help="Directory containing exported parquet files")
    parser.add_argument("--out-dir", required=True, help="Directory for per-address extracted JSON")
...
def build_extract(df: pd.DataFrame, address: str, top_n: int, sample_limit: int) -> dict[str, Any]:
    ...
    function_selectors = Counter(
        [
            str(v) for v in df["function_selector"].fillna("").astype(str).tolist()
            if str(v).strip() and str(v) != "0x"
        ]
    )
    ...
    if counterparties and tx_count:
        dominant_counterparty_share = round((counterparties.most_common(1)[0][1] / tx_count) * 100, 2)

    dominant_contract_share = 0.0
    if to_addresses and tx_count:
        dominant_contract_share = round((to_addresses.most_common(1)[0][1] / tx_count) * 100, 2)

    dominant_function_selector_share = 0.0
    if function_selectors and tx_count:
        dominant_function_selector_share = round((function_selectors.most_common(1)[0][1] / tx_count) * 100, 2)
        ...
        return {
            ...
            "summary": {
                ...
                "unique_function_selectors": len(function_selectors),
                "dominant_direction": dominant_direction,
                "dominant_counterparty_share": dominant_counterparty_share,
                "dominant_contract_share": dominant_contract_share,
                "dominant_function_selector_share": dominant_function_selector_share,
            },
...
def main() -> None:
    ...
    parquet_files = sorted(input_dir.rglob("part-*.parquet"))
    ...
    manifest = {
        "script": "summarize_optimism_transactions_parquet.py",
